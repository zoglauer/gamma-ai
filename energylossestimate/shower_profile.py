...
z_range = list(np.arange(z_vals_min, z_vals_max, z_step)) #[range(i, i+z_step, .1) for i in range(z_vals_min, z_vals_max, z_step)]
...
for event in event_list:
    ...
    for hit in event.hits:
        ...
        event.hits = np.append(event.hits, np.array(bins), 1)
